...
SerialCommunication::SerialCommunication(CommandFunctionStruct *commands) {
...
void SerialCommunication::executeCommand(char *token, String extra) {
    ...
    if (*token == '?') {
        ...
        for (CommandFunctionStruct *commandToupleP = commands; commandToupleP->commandName != ""; commandToupleP++) {
            Serial.print(commandToupleP->commandName);
            Serial.print(" - ");
            Serial.println(commandToupleP->commandHelp);
        }
...
//
void SerialCommunication::checkForCommand() {
